...
export function buildTypeListText(registry: TypeListRegistry, agentDir: string): string {
  ...
  return [
    ...
    "",
    `Custom agents can be defined in .pi/agents/<name>.md (project), ${agentDir}/agents/<name>.md (global), or a package-registered agent directory. Project-level agents override global ones, and both override package-provided agents. Creating a .md file with the same name as a default agent overrides it.`,
  ].join("\n");
}
