...
- Use model to specify a different model only for agents without a model in their agent definition.
- If you omit model, the subagent inherits the parent session's current model by default.
- Use thinking only for agents without a thinking level in their agent definition.
- For Nosh harness specialist agents, keep inherit_context false and pass context as named files in the prompt.
`,
			parameters: Type.Object({
				...
				subagent_type: Type.String({
					description: `The type of specialized agent to use. Available types: ${availableTypesText}. Custom agents from .pi/agents/<name>.md (project), ${agentDir}/agents/<name>.md (global), or package-registered agent directories are also available.`,
				}),
				model: Type.Optional(
					Type.String({
						description:
							'Optional model override for agents without a model in frontmatter. Accepts "provider/modelId" or fuzzy name (e.g. "haiku", "sonnet"). Omit to inherit the parent session model.',
					}),
				),
				thinking: Type.Optional(
					Type.String({
						description:
							"Thinking level: off, minimal, low, medium, high, xhigh. Used only when the agent definition does not set a thinking level.",
					}),
				),
				max_turns: Type.Optional(
					Type.Number({
						description:
							"Maximum number of agentic turns before stopping. Used only when the agent definition does not set max_turns. Omit for unlimited (default).",
						minimum: 1,
					}),
				),
				...
				inherit_context: Type.Optional(
					Type.Boolean({
						description:
							"If true, fork parent conversation into the agent. Default: false. Nosh harness specialist agents should keep this false and receive context through named files.",
					}),
				),
			}),
