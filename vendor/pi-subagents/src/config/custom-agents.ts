/**
 * custom-agents.ts — Load user-defined agents from package-registered, project
 * (.pi/agents/), and global ($PI_CODING_AGENT_DIR/agents/, default
 ...
 *   3. Package:  directories registered by another extension at runtime
 *
 * Project-level agents override global ones, and both override package-provided
 * agents with the same name.
 * Any name is allowed — names matching defaults (e.g. "Explore") override them.
 */
export function loadCustomAgents(cwd: string, packageDirs: string[] = []): Map<string, AgentConfig> {
  const globalDir = join(getAgentDir(), "agents");
  const projectDir = join(cwd, ".pi", "agents");

  const agents = new Map<string, AgentConfig>();
  for (const dir of packageDirs) {
    loadFromDir(dir, agents, "package"); // lowest priority
  }
  loadFromDir(globalDir, agents, "global");   // middle priority
