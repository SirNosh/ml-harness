...
export default function (pi: ExtensionAPI) {
  ...
  const registry = new AgentTypeRegistry(() => loadCustomAgents(process.cwd(), [...packageAgentDirs]));
  const registerAgentDirectory = (dir: string) => {
    packageAgentDirs.add(dir);
    registry.reload();
    return () => {
      packageAgentDirs.delete(dir);
      registry.reload();
    };
  };
  ...
  // Consumers: const { getSubagentsService } = await import("@nosh/pi-subagents");
  const service = new SubagentsServiceAdapter(manager, resolveModel, runtime, registerAgentDirectory);
